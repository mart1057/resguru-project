    ...
    <div>
        ...
        <div class=" bg-[white] pt-[14px] pb-[24px] pl-[24px] pr-[24px]  rounded-b-lg" v-if="tab == 1">
            <div class="grid grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4 mt-[14px]">
                <div class="bg-white rounded-[12px] h-[150px] border flex flex-col p-[12px]"
                v-for="data in UserBuilding" :key="data.id">
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-[12px]">
                                <div class="flex flex-col justify-between h-[100%]">
                                    ...
                                    <!-- Delete button moved to separate area -->
                                    <div class="flex justify-start">
                                        <div @click.stop="showAdminRemovalOptions(data.id, data.firstName + ' ' + data.lastName)" 
                ...
                <!-- Admin Removal Options Modal -->
                <b-modal centered v-model="adminRemovalModal" size="md" hide-backdrop hide-header-close hide-header hide-footer>
                    <div class="flex justify-end cursor-pointer" @click="adminRemovalModal = false">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <mask id="mask0_902_19193" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="24" height="24">
                                <rect width="24" height="24" fill="#D9D9D9" />
                            </mask>
                            <g mask="url(#mask0_902_19193)">
                    ...
                    <div class="mt-[14px]">
                        <h3 class="text-[20px] font-bold text-[#003765] mb-[12px]">จัดการแอดมิน</h3>
                        <p class="text-gray-600 mb-[20px]">{{ selectedAdminName }}</p>
                        
                        <!-- Loading state -->
                        <div v-if="checkingAdminBuildings" class="text-center py-4">
                            <div class="text-sm text-gray-500">กำลังตรวจสอบข้อมูล...</div>
                        </div>
                        
                        <!-- Removal Options -->
                        <div v-else class="space-y-3">
                            <!-- Remove from current building only -->
                            <div class="border rounded-[12px] p-[16px] cursor-pointer hover:bg-gray-50"
                                :class="{'opacity-50': !canRemoveFromBuilding}"
                                @click="canRemoveFromBuilding && confirmRemoveFromBuilding()">
                                <div class="flex items-start">
                                    <div class="mt-1">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M19 13H5v-2h14v2z" fill="#F59E0B"/>
                                        </svg>
                                    </div>
                                    <div class="ml-3 flex-1">
                                        <div class="font-bold text-[#F59E0B]">นำออกจากอาคารนี้เท่านั้น</div>
                                        <div class="text-sm text-gray-600 mt-1">
                                            {{ canRemoveFromBuilding ? 
                                                `แอดมินจะยังคงเป็นแอดมินในอาคารอื่น (${adminBuildingCount - 1} อาคาร)` : 
                                                'ไม่สามารถนำออกได้ เนื่องจากแอดมินอยู่ในอาคารนี้เท่านั้น' 
                                            }}
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="border border-red-200 rounded-[12px] p-[16px] cursor-pointer hover:bg-red-50"
                                ...
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-3 flex-1">
                                        <div class="font-bold text-red-600">ลบแอดมินออกจากระบบ</div>
                                        <div class="text-sm text-gray-600 mt-1">
                                            แอดมินจะถูกลบออกจากทุกอาคาร {{ adminBuildingCount > 1 ? `(${adminBuildingCount} อาคาร)` : '' }}
                        ...
                        <div class="mt-[20px] flex justify-center">
                            <button class="px-6 py-2 border border-gray-300 rounded-[8px] text-gray-700 hover:bg-gray-50"
                                    @click="adminRemovalModal = false">
                                ยกเลิก
                            </button>
                        </div>
                    </div>
...
export default {
    data() {
        return {
            ...
            // Admin removal related data
            adminRemovalModal: false,
            selectedAdminId: null,
            selectedAdminName: '',
            checkingAdminBuildings: false,
            adminBuildingCount: 0,
            canRemoveFromBuilding: false,
        }
    },
    components: {
    ...
    methods: {
        ...
        getUser() {
            ...
            fetch(`https://api.resguru.app/api/buildings/${this.$store.state.building}?populate=users_admin.imageProfile`)
                .then(response => response.json())
                .then((resp) => {
                    console.log("Building with related users:", resp);
                    if (resp.data?.attributes?.users_admin?.data) {
                        this.UserBuilding = resp.data.attributes.users_admin.data.map(user => ({
                            id: user.id,
                            ...user.attributes,
                            imageProfile: user.attributes.imageProfile?.data?.attributes
                        }));
                    }
                })
                .catch(error => {
        ...
        async showAdminRemovalOptions(adminId, adminName) {
            this.selectedAdminId = adminId;
            this.selectedAdminName = adminName;
            this.adminRemovalModal = true;
            this.checkingAdminBuildings = true;
            
            try {
                const currentUserId = this.$store.state.userInfo?.id;
                const response = await fetch(
                    `https://api.resguru.app/api/buildings?filters[user_owner][id][$eq]=${currentUserId}&populate=users_admin&filters[users_admin][id][$eq]=${adminId}`
                );
                
                const data = await response.json();
                this.adminBuildingCount = data.data ? data.data.length : 0;
                this.canRemoveFromBuilding = this.adminBuildingCount > 1;
                
            } catch (error) {
                console.error('Error checking admin buildings:', error);
                this.adminBuildingCount = 1;
                this.canRemoveFromBuilding = false;
            } finally {
                this.checkingAdminBuildings = false;
            }
        },

        confirmRemoveFromBuilding() {
            if (!this.canRemoveFromBuilding) return;
            
            const message = `คุณต้องการนำแอดมิน "${this.selectedAdminName}" ออกจากอาคารนี้เท่านั้น ใช่หรือไม่?\n\nแอดมินจะยังคงเป็นแอดมินในอาคารอื่นๆ`;
            
            if (confirm(message)) {
                this.removeAdminFromBuilding();
            }
        },

        confirmDeleteAdmin() {
            const buildingText = this.adminBuildingCount > 1 ? 
                `จากทุกอาคาร (${this.adminBuildingCount} อาคาร)` : 
                'ออกจากระบบ';
                
            const message = `⚠️ คำเตือน: คุณต้องการลบแอดมิน "${this.selectedAdminName}" ${buildingText} ใช่หรือไม่?\n\nการกระทำนี้ไม่สามารถกู้คืนได้`;
            
            if (confirm(message)) {
                this.deleteAdminCompletely();
            }
        },

        async removeAdminFromBuilding() {
            const loading = this.$vs.loading({
                text: 'กำลังนำแอดมินออกจากอาคาร...'
            });
            ...
            try {
                await axios.put(`https://api.resguru.app/api/buildings/${this.$store.state.building}`, {
                    data: {
                        users_admin: { 
                            disconnect: [this.selectedAdminId] 
                        }
                    }
                });
                
                this.$showNotification('#3A89CB', 'นำแอดมินออกจากอาคารสำเร็จ');
                this.adminRemovalModal = false;
                this.getUser();
                
            } catch (error) {
                console.error('Error removing admin from building:', error);
                const errorMessage = error.response?.data?.error?.message || 'เกิดข้อผิดพลาดในการนำแอดมินออกจากอาคาร';
                this.$showNotification('danger', errorMessage);
            } finally {
                loading.close();
            }
        ...
        async deleteAdminCompletely() {
            const loading = this.$vs.loading({
                text: 'กำลังลบแอดมินออกจากระบบ...'
            });
            
            try {
                await axios.delete(`https://api.resguru.app/api/users/${this.selectedAdminId}`);
                
                this.$showNotification('#3A89CB', 'ลบแอดมินออกจากระบบสำเร็จ');
                this.adminRemovalModal = false;
                this.getUser();
                
            } catch (error) {
                console.error('Error deleting admin completely:', error);
                const errorMessage = error.response?.data?.error?.message || 'เกิดข้อผิดพลาดในการลบแอดมิน';
                this.$showNotification('danger', errorMessage);
            } finally {
                loading.close();
            }
